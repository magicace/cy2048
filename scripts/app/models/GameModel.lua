...
GameModel.UNDO_EVENT	= "UNDO_EVENT"
...
function GameModel:flash(running)
	...
	--同步维护self.map表
	self:fillMap()
	if running then self:gameLog() end
...
function GameModel:undo()
	...
	if rec then
		...
		self:dispatchEvent({name=GameModel.UNDO_EVENT,cannot=true})
	end
end

function GameModel:saveGameData(flag)
